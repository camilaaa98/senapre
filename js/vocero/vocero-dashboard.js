...
const VoceroDashboard = (() => {
    ...
    // Estados inactivos que se excluyen del listado por defecto
    const ESTADOS_INACTIVOS = new Set(['CANCELADO', 'RETIRADO', 'APLAZADO', 'TRASLADO', 'FINALIZADO']);

    // Colores institucionales SENA y estados
    ...
    function init() {
        ...
        if (!scope) {
            _mostrarInfo('voc-tabla-body', 'No tiene una ficha asignada. Contacte al administrador.');
            return;
        }
    ...
    function _renderTablaEstados() {
        ...
        // Añadir estados no contemplados
        ...
        orden.forEach(est => {
            ...
            const dot = est.toLowerCase().replace('é', 'e');
            const dotClass = `dot-${dot}` in {} ? `dot-${dot}` : 'dot-total';
            filas += `<tr>
                <td>
                    <span class="voc-estado-dot dot-${dot}" title="${est}"></span>
                    <strong>${est}</strong>
                </td>
                <td>${cnt}</td>
                <td>${pct}%</td>
        ...
        document.getElementById('tabla-estados-body').innerHTML = filas;
    }
    ...
    // ── Renderizado de Tabla de Aprendices ─────────────────────
    function renderizarTabla() {
        ...
        // Filtrar: por defecto solo LECTIVA (activos)
        const lista = todosAprendices.filter(a => {
            const est = (a.estado || '').toUpperCase();
            const activo = mostrarTodos || (!estadoFilt && !ESTADOS_INACTIVOS.has(est))
        ...
        document.getElementById('voc-paginacion').innerHTML =
            _renderPaginacion(totalPags, paginaActual);
    }
    ...
    function exportarPDF() {
        ...
        doc.setFontSize(9);
        doc.text('RESUMEN POR ESTADO', 20, y + 6);
        y += 12;
        ...
        Object.entries(por).forEach(([estado, cnt]) => {
            ...
            doc.text(`${cnt}  (${pct}%)`, 80, y);
            y += 7;
        });
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(0, 50, 77);
        doc.text('TOTAL:', 22, y + 1);
        doc.text(`${total} aprendices`, 80, y + 1);
        y += 14;

        // Tabla de aprendices (solo activos en el PDF)
        ...
        doc.autoTable({
            ...
            columnStyles: {
                0: { cellWidth: 8 }, 1: { cellWidth: 26 }, 2: { cellWidth: 55 },
                3: { cellWidth: 48 }, 4: { cellWidth: 22 }, 5: { cellWidth: 22 }
            }
        });
    ...
    function _badgeEstado(estado) {
        const map = {
            LECTIVA: 'badge-lectiva',
            CANCELADO: 'badge-cancelado',
            RETIRADO: 'badge-retirado',
            APLAZADO: 'badge-aplazado',
            TRASLADO: 'badge-aplazado'
        };
...
// Funciones globales (puente HTML → módulo)
